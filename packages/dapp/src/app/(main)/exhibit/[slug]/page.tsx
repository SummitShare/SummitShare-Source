...
const data: ObjectDescription[] = [
  {
    ...
    Figure_artifact_details: "Mwenya Mukulu was the leader of the Lungu, Azamba, Azao or Sao people. The Lungu or Azamba people came down the lake in canoes from the eastern side of Lake Tanganyika at the end of the 17th century. They put to shore on the south end of the lake and Mwenya Mukulu became their Queen. She consolidated her leadership and the power among her people and the people of the new land. She then divided this area amongst her four sisters and made them rulers over the people they found there, founding the Lungu nation in the present day Mbala/Mpulungu areas of Zambia.",
    figure_images_URLs: [
      "https://s3.example.com/images/mwenya_1.jpg",
      "https://s3.example.com/images/mwenya_2.jpg"
    ],
    figure_details: [["Known as the 17th century Diplomat"],
    ["(1910 - 1986)"]],
    figure_biography: [
...
const getData = (slug: string) => {
  return data.find(item => item.title.toLowerCase().replace(/ /g, '-') === slug);
};
  ...
  return (
    <div className="mt-[100px] px-6 space-y-[48px]">
      ...
      <ul className='space-y-3'>
        ...
        <li><ul className='space-y-4 '> {figure.figure_details.map((desc, index) => (
          <li key={index}><p className='text-primary-100'>{desc}</p></li>
        ))}
          </ul></li>
      
      </ul>
        {figure.figure_biography.map((bio, index) => (
          <li key={index}><p>{bio}</p></li>
        ))}
      </ul>
