...
export async function POST(req: Request, res: NextResponse) {
    ...
    const { user_id, wallet_address }: { user_id: string, wallet_address: string } = requestBody;

    try {
        if (!user_id || !wallet_address) {
            return NextResponse.json({ error: 'Missing user_id or wallet_address' }, { status: 400 });
        }
