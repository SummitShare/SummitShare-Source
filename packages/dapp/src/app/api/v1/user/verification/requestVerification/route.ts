...
export async function GET(req: Request , res : NextResponse) {
   ...
   try {
    ...
    const verification =await createSendTokens(verificationRecord.user_id!, user?.email!)

      if (!verification){
        return NextResponse.json({ message: 'failed to create verification link',}, { status: 500 })
      }
