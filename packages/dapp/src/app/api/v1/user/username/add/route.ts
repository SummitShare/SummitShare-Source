...
export async function POST(req: Request, res: NextResponse) {
    try {
        ...
        if (existingUsername) {
            return NextResponse.json({ message: 'Username  unavailable already exists' }, { status: 409 });
        }
        ...
        if (user){
           const  user =  await prisma.users.update({
              ...
              return NextResponse.json({ message: 'Username updated', "username" :user.username }, { status: 200 });
        }
