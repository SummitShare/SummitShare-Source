...
export async function POST(req: Request, res: NextResponse) {
  try {
    ...
    if (!ticketTransaction) {
      return NextResponse.json({ message: "transaction not recorded", }, { status: 500 });
    }

    const MailSent = await sendEmail(email,"ticket purchased",`follow this link to view the event ${eventLink}`)
