...
async function updateStakeholderProposals(
...
async function ensureStakeholdersInTable(
  stakes: IStakes,
  proposal_id: string
): Promise<boolean> {
  ...
  for (const email of emails) {
    ...
    if (!CurrentStakeholder) {
      throw new Error(
        `Stakeholder not found for user ID: ${user.id} and event ID: ${proposal_id}`
      );
    }
  ...
  try {
    ...
    if (Object.keys(stakes).length === 0) {
      ...
      return NextResponse.json(
        { error: 'no stakeholders sent' },
        { status: 400 }
      );
    }

    const stakesValidated = await ensureStakeholdersInTable(
      stakes,
      proposal_id
    );
    if (stakesValidated == false) {
      return NextResponse.json(
        {
          error:
            'invalid or missing stakeholders please ensure all stakeholders have accepted the requesrs ',
        },
        { status: 400 }
      );
    }
    ...
    const status = await updateStakeholderProposals(
      proposal_id,
      newProposal.id
    );
    if (!status) {
      return NextResponse.json(
        { 'message ': 'error updating stakeholders' },
        { status: 500 }
      );
    }
