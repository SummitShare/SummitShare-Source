  ...
  try {
    ...
    if (allStakeholdersVotedPositively) {
      ...
      if (typeof proposal.content === 'string' && proposal.content !== null) {
        ...
        if (content.stakes) {
          try {
            const stakeUpdates = Object.entries(content.stakes).map(
              async ([email, stakeValue]) => {
                ...
                if (!stakeholder) {
                  throw new Error(
                    `Stakeholder not found for user ID: ${user.id} and event ID: ${event_id}`
                  );
                }
            ...
            return NextResponse.json(
              { error: 'An error occurred while updating stakeholders.' },
              { status: 500 }
            );
          }
        }
