...
const pathname = usePathname()
  ...
  return (
    <nav className='  w-full p-6 flex flex-row justify-between items-center border-b border-primary-900-5'>
      <div onClick={()=>setOpenMenu(!openMenu)} className='p-1 border border-primary-900 rounded-sm'><Bars3Icon className = "w-4"/></div>
            ...
            >
              ...
              <ul className='space-y-1'>
                {menu.items.map((subItem, subIndex) => (
                  <li
                    key={subIndex}
                    className={`text-xl md:text-base text-primary-700 font-normal ${pathname === subItem.link && 'text-primary-400'}`}
                  >
