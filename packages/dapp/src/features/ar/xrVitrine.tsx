...
const entryFailure = (error: unknown): UnsupportedReason => {
  ...
  if (name === 'NotSupportedError') {
    return {
      ...
      message: `The requested immersive AR feature set is unsupported (${formatError(error)}).`,
    }
  }
...
function ArtifactModel({
  ...
  // The loaded model is tagged with the artifact it belongs to. Holding a bare
  // Object3D meant that switching artifacts kept rendering the PREVIOUS one for
  // the whole load — seconds for a multi-megabyte Draco GLB on a phone — so
  // opening the drum, going back and opening the likishi showed the DRUM until
  // the new model landed, and showed it after the cleanup below had already
  // called dispose() on its GPU resources. Comparing the tag at render time
  // fixes that without clearing state inside the effect, which would cascade a
...
function PlacementContents({
  ...
  return (
    ...
    <group>
      <group position={[nudge.x, nudge.y, nudge.z]} rotation={[0, nudge.yaw, 0]}>
        <group
...
function AnchorVisibilityProbe({
  ...
  useFrame(() => {
    ...
    const visible =
      anchorRoot?.transformReady === true && anchorRoot.visible
    if (!visible) unlocatableFramesRef.current += 1
    const now = performance.now()
    if (visible === previous.current && now - reportedAtRef.current < 500) return
...
function HitTestPlacement({
  ...
  useFrame((_state, _delta, frame) => {
    ...
    if (!qualifyingResult || !qualifyingPose) {
      onPlacementMessage('No upward-facing hit within 20° was available on that frame.')
      return
    }
    ...
    void commitPlacement({
      ...
      .catch((error: unknown) => {
        if (lifecycleRefs.acceptingRef.current) onPlacementMessage(formatError(error))
      })
      .finally(() => {
    ...
    <>
      <group ref={reticle} matrixAutoUpdate={false} visible={false}>
        <mesh rotation={[-Math.PI / 2, 0, 0]}>
          ...
          <meshBasicMaterial color="#fb923c" transparent opacity={0.92} depthTest={false} />
        </mesh>
      </group>
      {placement && (
        <PlacementRoot
...
function XRScene({
  ...
  return (
    <XR store={store}>
      ...
      <directionalLight color={0xffffff} intensity={3.2} position={[1.5, 2.5, 2]} />
      <directionalLight color={0xffc16b} intensity={1.8} position={[-2, 1, 1]} />
      {/* Rim from behind, so the silhouette separates from the camera feed. */}
      <directionalLight color={0xffe6c2} intensity={1.1} position={[0, 1.5, -2.5]} />
      {/* Bounce, standing in for light coming back off the plinth. */}
      <directionalLight color={0xffd9a8} intensity={0.7} position={[0, -1.5, 0.5]} />
      {resources && (
        <>
          <HitTestPlacement
            resources={resources}
...
function XRVitrineInstance({
  ...
  const [capability, setCapability] = useState<CapabilityState>({ kind: 'checking' })
  const [failure, setFailure] = useState<UnsupportedReason | null>(initialFailure)
  ...
  const [placementPolicy, setPlacementPolicy] = useState<PlacementPolicy>('anchor')
      ...
      if (!window.isSecureContext) {
        setCapability({
          ...
          reason: {
            ...
            message: 'WebXR requires a secure context (HTTPS or same-device localhost).',
          },
        })
        return
      }
          ...
          supported
            ...
            : {
                ...
                reason: {
                  ...
                  message: 'immersive-ar is not supported on this device/browser.',
                },
              },
        )
      } catch (error) {
    ...
    try {
      ...
      if (!session.domOverlayState) {
        await failAfterEntry({
          ...
          message: 'DOM overlay was not granted for the immersive AR session.',
        })
        return
      }
      ...
      try {
        ...
        await failAfterEntry({
          ...
          message: `Viewer-space hit-test setup failed (${formatError(error)}).`,
        })
        return
      }
      ...
      if (!hitTestSource) {
        await failAfterEntry({
          kind: 'hit-test-unavailable',
          message: 'The browser could not create a viewer-space hit-test source.',
        })
        return
      }
      ...
      setPlacementMessage('Point at a flat surface until the circle appears, then tap.')
  ...
  const endSession = useCallback(() => {
    void storeRef.current?.getState().session?.end().catch(() => undefined)
  }, [])
  ...
  const unavailableReason =
    capability.kind === 'checking'
      ? ({ kind: 'checking', message: 'Checking immersive AR support…' } as const)
      : capability.kind === 'blocked'
        ? capability.reason
        : failure
          ? { ...failure, retry: requestStart }
          : store === null
            ? ({ kind: 'checking', message: 'Preparing the immersive AR viewer…' } as const)
            : null
        ...
        >
          <XRScene
            ...
            onAnchorVisibility={(visible, unlocatableFrames) => {
              ...
              setPlacementMessage(visible ? 'Anchor located.' : 'Anchor unlocatable.')
            }}
