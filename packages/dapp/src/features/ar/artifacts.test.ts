...
describe('AR artifact registry', () => {
   ...
   /**
    * The counterpart to the agreement guard above, for the one source allowed to
    * diverge. A 'vitrine' height is a deliberate enlargement for room-scale
    * legibility, so it must be STRICTLY greater than the marker-relative size.
    * That is what separates a declared choice from the 156 mm drum: an edit that
      ...
      for (const artifact of ARTIFACTS) {
         ...
         expect(
            ...
            `${artifact.slug} is labelled 'vitrine' but renders at ` +
               ...
               `${mechanical.toFixed(
                  4
               )} m MindAR renders. Either enlarge it or ` +
...
describe('absent medallion', () => {
   ...
   it('has no tracking target and no registry entry', () => {
      expect(
         existsSync(path.join(PUBLIC_DIR, 'ar', 'targets', 'absent.mind'))
      ).toBe(false);
