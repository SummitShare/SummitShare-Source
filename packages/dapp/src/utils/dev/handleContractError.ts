...
export const handleContractError = (error: any): string => {
  ...
  else if (error.code === 'UNPREDICTABLE_GAS_LIMIT' || error.code === ethers.errors.CALL_EXCEPTION) {
    message = 'Cannot estimate gas for this transaction. Please adjust the gas limit.';
  } 
  // User rejected the transaction
  else if (error.code === 4001 || (error.message && error.message.includes('user rejected transaction'))) {
    message = 'Transaction was rejected by the user.';
  } 
  // Network error or wallet not connected
  else if (error.code === ethers.errors.NETWORK_ERROR || (error.message && error.message.includes('wallet not connected'))) {
    message = 'Network error or wallet not connected. Please check your connection and try again.';
  }
  // Contract not deployed to network
  else if (error.message && error.message.includes('contract not deployed')) {
    message = 'The contract is not deployed on the current network. Please switch networks or contact support.';
  }
  // Transaction underpriced
  else if (error.message && error.message.includes('transaction underpriced')) {
    message = 'Transaction underpriced. Increase your gas price or gas limit and try again.';
  }
  // Transaction replacement underpriced
  else if (error.message && error.message.includes('replacement transaction underpriced')) {
    message = 'Replacement transaction underpriced. Increase the gas price for speeding up or cancelling the transaction.';
  }
  // Nonce too high
  else if (error.message && error.message.includes('nonce too high')) {
    message = 'Nonce too high. Please reset your account nonce or contact support.';
  }
  // Nonce too low
  else if (error.message && error.message.includes('nonce too low')) {
    message = 'Nonce too low. This transaction may replace or cancel a previous one. Please increase the nonce or wait for confirmation.';
  }
