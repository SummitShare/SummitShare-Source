...
function overwriteBigNumberFunction(functionName, readableName, readableNegativeName, _super, chaiUtils) {
    return function (...args) {
        ...
        if (Boolean(chaiUtils.flag(this, "doLength")) && (0, bigInt_1.isBigNumber)(actualArg)) {
            ...
            const actual = (0, bigInt_1.normalizeToBigInt)(actualArg);
            this.assert(compare(functionName, expected, actual), `expected ${expected} to ${readableName} ${actual}. The numerical values of the given "${(0, bigInt_1.formatNumberType)(expectedFlag).toString()}" and "${(0, bigInt_1.formatNumberType)(actualArg).toString()}" inputs were compared, and they differed.`, `expected ${expected} to ${readableNegativeName} ${actual}. The numerical values of the given "${(0, bigInt_1.formatNumberType)(expectedFlag).toString()}" and "${(0, bigInt_1.formatNumberType)(actualArg).toString()}" inputs were compared, and they differed.`, actual.toString(), expected.toString());
        }
        else {
            _super.apply(this, args);
        }
